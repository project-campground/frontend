...
const tentSidebarEventHandlers: Partial<{
	[K in keyof TypeToPayload]: (
		payload: TypeToPayload[K],
		bonfires: Record<string, GetTentsOutput>,
		tentSidebar: TentSidebar,
	) => void;
}> = {
	// Bonfires
	BonfireDeleted(payload, bonfires, tentSidebar) {
		...
		if (tentSidebar.bonfireSelected.id !== payload.id) return;

		const cached = Object.keys(bonfires);

		// Navigate to last known cached tent
		tentSidebar.props.navigate(
			`/c/${tentSidebar.props.campsite.id}/t/${cached.length ? bonfires[cached[0]].tents[0].id : `bulletin`}`,
		);
	},
	// Tents
	...
	TentUpdated(payload, bonfires) {
		...
		if (existingTentModified) Object.assign(existingTentModified, payload);
	},
	TentMoved(payload, bonfires) {
		...
		const otherTentsInCategory = bonfires[payload.bonfireId]?.tents.filter(
			(x) => x.id !== payload.id && x.categoryId === payload.categoryId,
		);

		makeRoomForItems(payload, otherTentsInCategory);

		const previousTentState = Object.values(bonfires)
			.flatMap((x) => x.tents)
		...
		if (!previousTentState) return bonfires[payload.bonfireId]?.tents.push(payload);
		...
		bonfires[previousTentState.bonfireId].tents.splice(
			bonfires[previousTentState.bonfireId].tents.findIndex((x) => x.id === payload.id),
			1,
		);
	...
	CategoryUpdated(payload, bonfires) {
		const existingCategoryModified = bonfires[payload.bonfireId].categories.find(
			(x) => x.id === payload.id,
		);

		if (existingCategoryModified) Object.assign(existingCategoryModified, payload);
	},
	CategoryMoved(payload, bonfires) {
		// Move other tents to make room for the new position; this is done in the back-end, but not notified directly
		const otherCategories = bonfires[payload.bonfireId]?.categories.filter(
			(x) => x.id !== payload.id,
		);

		makeRoomForItems(payload, otherCategories);

		const previousCategoryState = Object.values(bonfires)
		...
		const tentsToAlsoMove = bonfires[previousCategoryState.bonfireId].tents.filter(
			(x) => x.categoryId === previousCategoryState.id,
		);

		bonfires[previousCategoryState.bonfireId].categories.splice(
			bonfires[previousCategoryState.bonfireId].tents.findIndex((x) => x.id === payload.id),
			1,
		);
		// Remove all category's tents from the bonfire
		bonfires[previousCategoryState.bonfireId].tents = bonfires[
			previousCategoryState.bonfireId
		].tents.filter((x) => x.categoryId !== previousCategoryState.id);
...
export function makeRoomForItems<T extends { id: string; position: number }>(
	...
	if (others?.some((x) => x.id !== payload.id && x.position === payload.position)) {
		for (const item of others.filter((x) => x.position >= payload.position)) item.position++;
	}
}
