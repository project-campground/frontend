...
const MessageEditorGroup = styled(Group)(() => ({ height: '100%' }));
...
export default function MessageEditor({
	...
	const [editor] = useState(
		() => withCgMarkdown(withHistory(withReact(createEditor()))) as RichEditor,
	);
	...
	const onDone = async () => {
		if (
			...
			&& (editor.children[0] as EditorElement).type === 'paragraph'
			&& !Node.string(editor.children[0]).trim()
		)
			return;
			...
			<MessageEditorContainer>
				...
				>
					...
					>
						<Dropdown>
							<MenuButton
								slots={{ root: MessageEditorButton }}
						...
						<Box sx={{ maxHeight: 200, overflowY: 'auto', overflowX: 'hidden', flex: 1 }}>
							<TextEditor
								...
								keyboardSettings={{ enterCallback: onDone, escapeCallback: onClearReplies }}
							/>
						</Box>
						<Group
							gap={2}
