...
export async function clientLoader({
	...
	const invite = await session.http.invitesGlobal.get(
		inviteDomain || defaultBackendDomain,
		inviteId,
	);
	...
	if (!invite.ok)
		return {
			err: invite.status,
			errorDescription: invite.errorDescription,
			errorHeader: invite.errorHeader,
			inviteId,
			invite: null,
		};

	return { err: null, inviteDomain, inviteId, invite: invite.content };
}

export default function Index({
	loaderData: { invite, inviteId, inviteDomain, err, errorDescription, errorHeader },
}: Route.ComponentProps) {
	if (err)
		return (
			<PagePlaceholder
				icon={err === 404 ? PagePlaceholderIcon.NotFound : PagePlaceholderIcon.Error}
				title={`${err} ${errorHeader}`}
			>
				{errorDescription}
			</PagePlaceholder>
		);
	...
	return (
		<CampsiteInvite
			inviteId={inviteId}
			domain={inviteDomain || defaultBackendDomain}
			invite={invite!}
		/>
	);
}
