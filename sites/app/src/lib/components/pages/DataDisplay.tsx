...
type Props<T> = Omit<DataTableProps<T>, 'values' | 'selected'>
	& Omit<DataCardStackProps<T>, 'values' | 'selected'> & {
		title: string;
		itemsPerPage: number;
		...
		updateItems?: <TEvent extends keyof TypeToPayload>(
			items: T[],
			type: TEvent,
			payload: TypeToPayload[TEvent],
		) => boolean;
	};
type State<T> = { page: number; items: T[]; itemsSelected: T[] };
const SmallScreens = styled(`div`, { name: 'DataDisplay', slot: 'small' })(({ theme }) => ({
	display: 'none',
	[theme.breakpoints.down('lg')]: { display: 'block' },
}));
const MediumScreens = styled(`div`, { name: 'DataDisplay', slot: 'medium' })(({ theme }) => ({
	display: 'none',
...
export default class DataDisplay<T> extends React.Component<Props<T>, State<T>> {
	...
	public state: State<T> = { page: 0, items: [], itemsSelected: [] };
	...
	constructor(props: Props<T>, context: State<T>) {
		...
		this._maxPage =
			this.props.maxItems !== null ? Math.ceil(this.props.maxItems / this.props.itemsPerPage) : null;
	}
	...
	public async componentDidMount(): Promise<void> {
		...
		if (this.props.updateItems) this.subscribeToWs();

		return this.fetchData();
	}

	public async componentDidUpdate(
		_prevProps: Readonly<Props<T>>,
		prevState: Readonly<State<T>>,
		_snapshot?: any,
	): Promise<boolean | void> {
		if (prevState.page === this.state.page || this._lock) return;
	...
	public componentWillUnmount(): void {
		if (this._wsSubscription) this.context.session.ws.unsubscribe(this._wsSubscription);
	}
	...
	private subscribeToWs() {
		...
		this._wsSubscription = session.ws.subscribe(
			(ws) =>
				ws.op === 1
				&& this.props.updateItems?.(this.state.items, ws.t, ws.payload)
				&& this.setState({}),
		);
	}
	...
	private async fetchData() {
		...
		this.props
			.fetch(this.state.page * this.props.itemsPerPage, this.props.itemsPerPage)
			.then((resp) => {
	...
	private onSelectAll() {
		this.setState({
			itemsSelected:
				this.state.itemsSelected.length === this.state.items.length ? [] : [...this.state.items],
		});
	}
		...
		return (
			...
			>
				<Group alignItems='center'>
					{menu && (
						<Box
							sx={{
								...
								opacity: itemsSelected.length && 1,
								overflow: 'hidden',
								width: itemsSelected.length && 48,
							}}
						>
							<Dropdown>
								<MenuButton
								...
								<Menu variant='soft'>
									{menu.map((x, i) => (
										<MenuItem
											...
											color={x.color}
											variant={x.variant}
											onClick={() => x.onClick(itemsSelected)}
										>
											{x.startDecorator && <ListItemDecorator>{x.startDecorator}</ListItemDecorator>}
											<ListItemContent>{x.content}</ListItemContent>
											{x.endDecorator && <ListItemDecorator>{x.endDecorator}</ListItemDecorator>}
										</MenuItem>
									))}
								</Menu>
							</Dropdown>
					...
					>
						<Skeleton loading={this._lock}>
							{itemsSelected.length || (maxItems ?? items.length)} {title}{' '}
						...
						<MediumScreens>
							<DataTable
								...
								columns={columns.map((col) => ({
									...
									name: (
										<Typography>
											<Skeleton loading>{col.name}</Skeleton>
										</Typography>
									),
				...
				>
					<Typography>
						<Skeleton loading={this._lock}>
							Showing {items.length} {title} of {maxItems} total {'\u2014'} from {page * itemsPerPage + 1}{' '}
							to {page * itemsPerPage + items.length}
						</Skeleton>
					</Typography>
					<ButtonGroup>
						<Button
						...
						{page !== 0 && <Button onClick={() => this.setState({ page: 0 })}>1</Button>}
						<Button disabled>{page + 1}</Button>
						{maxPage !== null && maxPage > page + 1 && (
							<Button onClick={() => this.setState({ page: maxPage - 1 })}>{maxPage}</Button>
						)}
						<Button
							onClick={() => this.setState({ page: page + 1 })}
							disabled={
								(maxPage === null && items.length < itemsPerPage)
								|| (maxPage !== null && (page + 1 === maxPage || maxPage < 2))
							}
						>
							<IconCaretRightFilled />
						</Button>
...
function SkeletonComponent() {
	return (
		...
		>
			<Skeleton loading>{loremIpsum.sm}</Skeleton>
		</Typography>
	);
}
