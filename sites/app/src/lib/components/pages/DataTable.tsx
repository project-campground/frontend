...
const DataTableRoot = styled(Table, { name: 'DataTable', slot: 'root' })(({ theme }) => ({
	borderRadius: theme.vars.radius.md,
	border: 'solid 1px transparent',
	'--TableCell-paddingY': theme.spacing(1.5),
	'--TableCell-paddingX': theme.spacing(1),
}));
const TableRow = styled('tr', { name: 'DataTable', slot: 'row' })(() => ({}));
const TableColumn = styled('td', { name: 'DataTable', slot: 'column' })(({ theme }) => ({
	[theme.breakpoints.keys.map((x) => `&.${x}`).join(', ')]: { display: 'none' },
	[theme.breakpoints.up('xs')]: { '&.xs': { display: 'table-cell' } },
	[theme.breakpoints.up('sm')]: { '&.sm': { display: 'table-cell' } },
	[theme.breakpoints.up('md')]: { '&.md': { display: 'table-cell' } },
	[theme.breakpoints.up('lg')]: { '&.lg': { display: 'table-cell' } },
	[theme.breakpoints.up('xl')]: { '&.xl': { display: 'table-cell' } },
}));
const TableHeadColumn = styled('th', { name: 'DataTable', slot: 'headColumn' })(({ theme }) => ({
	[theme.breakpoints.keys.map((x) => `&.${x}`).join(', ')]: { display: 'none' },
	[theme.breakpoints.up('xs')]: { '&.xs': { display: 'table-cell' } },
	[theme.breakpoints.up('sm')]: { '&.sm': { display: 'table-cell' } },
	[theme.breakpoints.up('md')]: { '&.md': { display: 'table-cell' } },
						...
						{!unselectable && (
							<TableHeadColumn style={{ width: 48 }}>
								<Checkbox
									variant='soft'
									color={
										selectedValues.length === values.length && selectedValues.length ? 'success'
