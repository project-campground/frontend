import { Box, Stack, Tabs, Skeleton, } from "@mui/joy";
import type {
    ProfileViewEmpty,
...
import ProfileFeedPost, {
    ProfileFeedPostReplySkeleton,
} from "./ProfileFeedPost";
...
export default function ProfileFeed({ user }: Props) {
    ...
    const isSelf =
        session.auth.authenticated && session.auth.user.did === user.did;

    useEffect(() => {
        setIsLoading(true);
        api.profilePosts.getMany(user.did, fetchReplies).then((posts) => {
            if (!posts.ok)
                return setError(
                    new HTTPError(
                        posts.errorDescription,
                        posts.status,
                        posts.errorHeader,
                    ),
                );
    ...
    const onPostCreated = (content: string) => {
        ...
        return session.atproto.profilePostRecords
            .create(newPost)
            .then((x) =>
                setPostList([
                    {
                        ...
                        parent: null,
                    } satisfies ProfilePostViewParented,
                    ...postList,
                ]),
            )
    ...
    const onPostDeleted = (uri: string) => {
        return session.atproto.profilePostRecords
            ...
            .catch((e) =>
                console.error("Got an error while deleting a post", e),
            );
    };
    const onPostUpdated = (uri: string, content: string) => {
        return session.atproto.profilePostRecords
            ...
            .catch((e) =>
                console.error("Got an error while editing a post", e),
            );
    };
        ...
        return (
            ...
            >
                <FormattedMessage
                    ...
                    defaultMessage="The user has deactivated their account, deleted it or profile does not exist."
    ...
    return (
        <Box sx={{ px: 2 }}>
            ...
            <Tabs
                onChange={(_, v) => setFetchReplies(Boolean(v))}
                size="lg"
                sx={{ mb: 2 }}
            >
                <SmoothTabList
                    tabs={[
                        {
                            id: 0,
                            name: <FormattedMessageGlobal id="app.profiles.feed" />,
                            startDecorator: <IconFlameFilled />,
                        },
                        {
                            id: 1,
                            name: <FormattedMessageGlobal id="app.profiles.replies" />,
            ...
            {isLoading ? (
                <ProfileFeedPostsSkeleton />
            ) : (
                <>
                    {!fetchReplies &&
                        isSelf &&
                        (account.sessionInfo.active ||
                            typeof account.sessionInfo.active ===
                                "undefined") && (
                            <ProfilePostCreator
                    ...
                    <PagePlaceholder
                        ...
                        title="No more posts"
                    >
                        This user has no more posts to be found! Come back
                        later!
                    </PagePlaceholder>
                </>
            )}
...
function ProfileFeedPostsSkeleton() {
    return (
        <>
            <Stack gap={2}>
                <ProfileFeedPostReplySkeleton />
