...
export interface FormFieldSwitchProps
	extends FormFieldProps<boolean | number>, FormFieldDecoratorProps {
...
type State = { value: boolean | number };

export default class FormFieldSwitch extends AbstractFormField<
	boolean | number,
...
> {
	...
	private onInputChange(ev: React.ChangeEvent<HTMLInputElement>) {
		...
		const checkedValue =
			this.props.checkedValue ? (Number(value) as 1 | 0) * this.props.checkedValue : value;
		...
		return (
			<FormControl orientation='horizontal'>
				...
				{design === 'button' ?
					<Sheet
						variant='soft'
						sx={{ px: 3, py: 1.5, borderRadius: 'md', lineHeight: 0 }}
					>
						<Switch
							variant='soft'
							size='lg'
