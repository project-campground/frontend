...
const settingsPages: Record<
	...
	| ((props: SettingsComponentProps<CategorySettingsProps>) => ReactNode | ReactNode[])
> = {
...
export default function CategorySettingsModal(props: CategorySettingsProps) {
	...
	const callbacks: Record<CategorySettingsPage, (fieldValues: Record<string, any>) => unknown> = {
		profile: (fieldValues) =>
			session.atproto.categories
				.update(props.category.id, { name: fieldValues.name, description: fieldValues.description })
				.then(handleAnyRestErrorWith(snackbars)),
		permissions: ({ roleId, userId, permissions }) =>
			session.atproto.permissions
