...
export type TextTentMessage = MessageViewWithReplies & { waiting?: true; error?: string };
...
export default class TextTent extends React.Component<Props, State> {
	...
	private onWebSocketEvent(type: string, payload: any) {
		...
		switch (type) {
			case 'MessageCreated':
					...
					this.state.messages.some(
						(x) =>
							...
							|| (x.waiting
								...
								&& session.auth.user.did === message.createdBy.user.did),
					)
				)
					return;
				...
				this.setState({
					messages: [
						{ ...message, replyingTo: [], replyingToCount: message.replyingTo.length },
						...this.state.messages,
					],
				});
				break;
	...
	async componentDidUpdate(
		...
		if (!this._initLock || this._lock || prevProps.tent.id === this.props.tent.id) return;
		...
		return this.fetchMessages(0).then(
			(messages) =>
				messages
				&& !(this._lock = false)
				&& this.setState({ messages, isEnd: messages.length < 50, loading: false }),
		);
	}
	...
	componentWillUnmount(): void {
		...
		(this.context as CampsiteContext).session.ws.unsubscribe(this._wsSubscription);
	}
	...
	async onMessageCreate(content: string): Promise<unknown> {
		...
		const fakeMessage: TextTentMessage = {
			...
			replyingTo: replyMessages.map(
				(x) => ({ ...x, replyingTo: x.replyingTo.map((y) => y.id) }) as MessageViewBasic,
			),
		...
		return api.messages
			.create(this.props.tent.id, { content, replies: replyMessages.map((x) => x.id) })
			.then((resp) => {
				if (!resp.ok)
					return this.setState({
						messages: [
							{ ...fakeMessage, error: resp.errorDescription },
							...this.state.messages.filter((x) => x.id !== fakeMessage.id),
						],
					});
	...
	async deleteMessage(messageDeleted: MessageViewWithReplies) {
		...
		if (this.pseudoMessages.includes(messageDeleted.id))
			return this.setState({
				deleteMessage: null,
				messages: this.state.messages.filter((x) => x.id !== messageDeleted.id),
			});
		...
		return (
			<Stack sx={{ height: '100%', overflow: 'hidden' }}>
				...
				<MessageInputWrapper
					...
					colorRoles={colorRoles}
					canCreate={Boolean(
						this.context.permissions.getTentPermissions(tent.categoryId, tent.id).content
						& ContentPermissionConsts.CREATE_CONTENT,
					)}
				/>
				<ContentDeleteModal
					nominativeCase={<FormattedMessageGlobal id='app.messages.nominativeCase' />}
					accusativeCase={<FormattedMessageGlobal id='app.messages.accusativeCase' />}
...
type MessageListProps = {
	...
	onMessageDelete: (message: MessageViewWithReplies, prompt: boolean) => unknown;
...
function MessageList({
	...
	const onScroll = (e: React.UIEvent<HTMLDivElement, UIEvent>) => {
		...
		if (-target.scrollTop < target.scrollHeight - target.offsetHeight - 80) return;
		...
		<>
			<MessageLimitStack onScroll={isEnd ? undefined : onScroll}>
				...
				{isEnd ?
					...
					>
						<FadingBox
							sx={{ zIndex: 1, position: 'absolute', left: 0, right: 0, bottom: 40, opacity: 0.15 }}
						>
							<Group px={4}>
								<Typography
									level='title-lg'
