...
export interface FormFieldSelectProps
	extends FormFieldProps<string | boolean | number | null>, FormFieldDecoratorProps {
	options: FormFieldSelectItem[];
	placeholder?: string;
}

type State = { value: string | number | boolean | null };

export default class FormFieldSelect extends AbstractFormField<
...
> {
	...
	private renderOption(
		...
		option: FormFieldSelectItem | undefined,
	) {
		// console.log({ value, option });
		return (
			<>
				{option?.startDecorator && <ListItemDecorator>{option?.startDecorator}</ListItemDecorator>}
				{option?.text ?? value}
				{option?.endDecorator && <ListItemDecorator>{option?.startDecorator}</ListItemDecorator>}
			</>
		);
	}
