...
const DataCardStackRoot = styled(Stack, { name: 'DataTable', slot: 'root' })(({ theme }) => ({
	gap: theme.spacing(1),
}));
...
function DataCardStackCard<T>({
	...
	HeaderComponent,
	Component,
}: { item: T; toggleSelect: () => unknown; selected: boolean } & Pick<
	DataCardStackProps<T>,
	'unselectable' | 'HeaderComponent' | 'Component'
>) {
	...
	return (
		<Card>
			<Stack gap={1}>
				...
				<Box
					sx={{ transition: 'height 0.3s', overflow: 'hidden', height: expanded ? 'min-content' : 0 }}
				>
