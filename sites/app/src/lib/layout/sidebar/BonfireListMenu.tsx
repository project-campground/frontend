...
export default function BonfireListMenu({ campsiteId, top, open, bonfires, onBonfireOpen }: Props) {
	...
	const lowestPriorityBonfire =
		bonfires.sort((a, b) => a.position - b.position).slice(-1)[0]?.position ?? -1;
	...
	const onDropped = (
		...
		if (bonfireDescended.includes(draggableGroup as 'tent' | 'category'))
			return api[draggableGroup === 'tent' ? 'tents' : 'categories']
				.move(draggedId, { bonfireId: droppedId, categoryId: null })
		...
		return (
			...
			&& api.bonfires
				.move(draggedId, { position: bonfireMovedToPosition })
				.then(handleAnyRestErrorWith(floating))
		);
	};
	...
	return (
		<>
			...
			>
				...
				<MenuItem
					sx={(theme) => ({ border: `dashed 1px ${theme.vars.palette.neutral[700]}` })}
