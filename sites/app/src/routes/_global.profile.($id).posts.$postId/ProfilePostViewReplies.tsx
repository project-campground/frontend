...
import type { ProfilePostView, ProfilePostViewBasic } from 'types/campground/user';
import ProfilePost from '../_global.profile.$id/ProfilePost';
import { useSession } from '~/context/session';
import { useAccount, type AccountContextAuthenticated } from '~/context/account';
import ProfilePostCreator from '../_global.profile.$id/ProfilePostCreator';
import { ThreadLineItem } from '../../lib/components/ThreadLine';

type RepliesProps = { post: ProfilePostView; topReplies: ProfilePostViewBasic[] };

export default function ProfilePostViewReplies({ post, topReplies }: RepliesProps) {
	const account = useAccount();
	// TODO: Scroll below to add more replies
	const [replies, setReplies] = useState(topReplies);
	const session = useSession();
	useEffect(() => setReplies(topReplies), [topReplies]);
	...
	const onCommentUpdated = (uri: string, content: string) =>
		session.atproto.profilePostRecords.update(uri, { content })?.then(() => {
					...
					setReplies([
						{
							...newPost,
							author: {
								...(account as AccountContextAuthenticated).profile,
								handle: (account as AccountContextAuthenticated).sessionInfo.handle,
								did: (account as AccountContextAuthenticated).sessionInfo.did,
							},
							replyCount: 0,
							uri: x.content.uri,
