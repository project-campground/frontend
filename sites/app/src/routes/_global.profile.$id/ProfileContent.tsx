...
const ProfileContentRoot = styled(Box, { name: 'ProfileContent', slot: 'root' })(({ theme }) => ({
	display: 'grid',
	flex: 1,
	gap: theme.spacing(6),
	gridTemplateColumns: '11fr',
	padding: 0,
	[theme.breakpoints.up('md')]: { padding: `0 ${theme.spacing(16)}` },
	[theme.breakpoints.up('lg')]: {
		padding: `0 ${theme.spacing(20)}`,
		gridTemplateColumns: '2fr 7fr 2fr',
	},
	[theme.breakpoints.up('xl')]: { padding: `0 ${theme.spacing(24)}` },
}));
const ProfileSideColumn = styled(Box, { name: 'ProfileContent', slot: 'sideColumn' })(
	({ theme }) => ({ [theme.breakpoints.down('lg')]: { display: 'none' } }),
);
...
export function ProfileContentSkeleton() {
	return (
		<ProfileContentRoot>
			<ProfileSideColumn></ProfileSideColumn>
			<Box>
				<ProfileFeedSkeleton />
			</Box>
			<ProfileSideColumn></ProfileSideColumn>
