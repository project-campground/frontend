...
const ActorMentionWrapper = styled('span', { name: 'ActorMention', slot: 'root' })(({ theme }) => ({
	display: 'inline-flex',
	flexDirection: 'row',
	alignItems: 'center',
...
export default function ActorMention({ did }: Props) {
	...
	useMemo(() => {
		return session.atproto.members
			.get(campsite.id, did)
			.then((resp) => (resp.ok ? setMember(resp.content) : null));
	}, [did]);
	const colorRole =
		member ? campsite.roles.find((x) => member.roles.includes(x.id) && x.colors.length) : null;
