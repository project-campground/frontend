...
type State = { error: ErrorBoundaryError | null };
...
export default class ErrorBoundary extends React.Component<PropsWithChildren, State> {
				...
				<PagePlaceholder
					...
					title={
						<TextBlock>
							...
							:	null}
							<TextBlock>{error.header}</TextBlock>
						</TextBlock>
					}
				>
					{error.message}
	...
	static getDerivedStateFromError(error: Error): { error: ErrorBoundaryError } {
		if (error instanceof HTTPError)
			return { error: { message: error.message, header: error.header, status: error.status } };

		return { error: { message: error.message, header: error.name, status: null } };
	}
}
