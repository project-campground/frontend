...
import type { ProfilePostViewDetailed } from 'types/campground/user';
...
type Props = { profileId: string; postId: string };
...
export default function ProfilePostView({ profileId, postId }: Props) {
	...
	useEffect(() => {
		api.profilePosts.get(profileId, postId).then((resp) => {
			if (!resp.ok) return setError(resp);
			return setPost(resp.content);
		});
	}, [profileId, postId]);

	if (error) return <PagePlaceholderFromApi response={error} />;

	const onPostDeleted = (uri: string) =>
		session.atproto.profilePostRecords.delete(uri).then((resp) => {
	...
	const onPostUpdated = (uri: string, content: string) =>
		session.atproto.profilePostRecords.update(uri, { content }).then((resp) => {
			if (!resp.ok) return floating.notifyApiError(resp);
