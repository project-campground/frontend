...
type State = {
    value: MapValue;
    valid: Record<string | number, boolean>;
};

export default class FormFieldObject extends AbstractFormField<MapValue, FormFieldObjectProps, State> {
    ...
    private onSubFieldChange = (id: string | number, isValid: boolean, value: any) =>
        this.setState(
            {
                value: { ...this.state.value, [id]: value },
    ...
    public override render(): ReactNode {
        ...
        return (
            <Stack gap={2}>
                ...
                >
                    <FormBody className={clsx("Form-body", inline && "Form-body-inlineContent")} gap={gap}>
                        {children}
                    </FormBody>
                </FormContext.Provider>
            </Stack>
