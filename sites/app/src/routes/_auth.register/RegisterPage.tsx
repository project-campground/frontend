...
type Props = { defaultDescribedServer: DescribedServer };
...
export default function RegisterPage({ defaultDescribedServer }: Props) {
	...
	const [serverDescription, setServerDescription] =
		useState<DescribedServer>(defaultDescribedServer);
	const [error, setError] = useState<null | HttpResponseError>(null);

	const onSubmit = async (
		event: React.MouseEvent<Element, MouseEvent> | undefined,
		{ confirmPassword: _, pds, handle, ...fieldValues }: Record<string, any>,
	) => {
		...
		session.setAuth({
			...
			user: { email: fieldValues.email, emailConfirmed: false, active: true, ...account.content },
		});
		return navigate('/');
	};
	const checkIfDomainIsPds = async (domain: string) => {
		...
		return HTTPAtprotoClient.describeServer({ url: `http${isLocalhost ? '' : 's'}://${domain}` })
			.then((resp) => {
			...
			<FormSection>
				<FormControl required>
					...
					<FormFieldText
						...
						knownValues={[
							{ startDecorator: <IconWorld />, value: defaultDomain, content: defaultDomain },
						]}
						id='pds'
			...
			<FormSection>
				<FormControl required>
					...
					<FormFieldText
						...
						max={253 - serverDescription.availableUserDomains[0].length}
						format={/^([A-Za-z0-9$_.-]+)$/}
						startDecorator={
							<Stack
								direction='row'
