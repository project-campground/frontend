...
export const RightSidebarBox = styled(Stack, { name: 'CampsiteSidebar', slot: 'wrapper' })(() => ({
	gap: 4,
	height: '100%',
	transition: 'width 0.3s',
	'.closed': { width: 0 },
}));
export const RightSidebarList = styled(Stack, { name: 'CampsiteSidebar', slot: 'root' })(
	({ theme }) => ({
...
export default class MemberSidebar extends React.Component<Props, State> {
	static contextType?: React.Context<any> | undefined = CampsiteContext;
	declare context: ContextType<typeof CampsiteContext>;

	private init: boolean = false;
	state: State = { loading: true, end: false, error: null, tab: 0, members: [] };
	...
	async fetchMembers(offset: number) {
		return this.context.api.members.getMany(this.props.campsiteId, offset);
	}
		...
		return (
			<RightSidebarBox className={closed ? 'closed' : ''}>
				...
				<RightSidebarList flex={1}>
					<Tabs
						onChange={(_, v) => this.setState({ tab: (v ?? 0) as number })}
						size='lg'
						sx={{ mb: 1, borderRadius: 'xl', overflow: 'hidden' }}
					>
						<SmoothTabList
							tabs={[
								{ id: 0, startDecorator: <IconUsers />, name: <FormattedMessageGlobal id='app.members' /> },
								{
									id: 1,
									startDecorator: <IconListTree />,
									name: <FormattedMessageGlobal id='app.threads' />,
								},
							]}
						/>
					</Tabs>
