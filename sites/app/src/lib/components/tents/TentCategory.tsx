...
type Props<T extends TentCategoryViewComponent> = React.PropsWithChildren & {
	...
	onSettingsOpen?: (props: {
		tent?: TentViewBasic;
		category?: T;
		page?: CategorySettingsPage;
	}) => unknown;
};

export default function TentCategory<T extends TentCategoryViewComponent>({
	onCreate,
	category,
	children,
	onSettingsOpen,
	...
	const categoryPermissions =
		permissions.permissions.categories[category.id] ?? permissions.permissions.bonfire;
	...
	const { listeners } = useRightClick({
		MenuComponent: () => (
			<>
				{canManageCategory && onSettingsOpen && (
					<MenuItem onClick={() => onSettingsOpen({ category })}>
						<ListItemDecorator>
							<IconSettingsFilled />
						</ListItemDecorator>
						<ListItemContent>
							<FormattedMessageGlobal id='app.tentCategories.settings' />
						</ListItemContent>
					</MenuItem>
				)}
				{canManageCategory && onSettingsOpen && (
					<MenuItem
						color='danger'
						variant='plain'
						onClick={() => onSettingsOpen({ category, page: 'delete' })}
					>
						<ListItemDecorator>
							<IconTrashFilled />
						</ListItemDecorator>
