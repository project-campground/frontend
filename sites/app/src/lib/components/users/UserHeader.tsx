...
export function UserHeaderBanner({
	isLoading,
	src,
	aspectRatio,
	borderRadius,
	...
	return (
		...
		>
			...
			: src ?
				<Image
					src={src}
					sx={{ zIndex: 'inherit', width: '100%', height: '100%' }}
				/>
			:	<GradientBanner
					color='primary'
					sx={{ zIndex: 'inherit', width: '100%', height: '100%' }}
...
export function UserHeaderAvatar({
	isLoading,
	src,
	did,
}: {
	...
	return isLoading ?
			<UserAvatarSkeleton
				withStatus
				size='xxl'
				sx={(theme) => ({ border: `solid 4px ${theme.vars.palette.background.level2}` })}
			/>
		:	<UserAvatar
				withStatus
				avatar={src}
				did={did}
				size='xxl'
				sx={(theme) => ({ border: `solid 4px ${theme.vars.palette.neutral.softBg}` })}
			/>;
}
