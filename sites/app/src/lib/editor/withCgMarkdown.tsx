...
const unorderedList: PrefixedElement = { type: 'list-item', wrapper: 'unordered-list' } as const;
...
function getPossibleOrderedList(text: string): PrefixedElement | null {
	...
	// "2. "
	if (Number.isNaN(num) || num < 0 || split[1] !== ' ') return null;
...
function createListFromPrefix(editor: RichEditor, elem: PrefixedElement) {
	Transforms.wrapNodes(
		editor,
		{ type: elem.type, children: [] },
		{ match: (n) => !Editor.isEditor(n) && Element.isElement(n) && n.type === 'paragraph' },
	);
	Transforms.wrapNodes(
		editor,
		{ type: elem.wrapper, children: [], ...elem.wrapperData },
		{ match: (n) => !Editor.isEditor(n) && Element.isElement(n) && n.type === elem.type },
	);
...
function modifiedInsertText(editor: RichEditor, text: string): boolean {
	...
	if (
		!endingCharacters.some((x) => text.endsWith(x))
	...
	if (elem.type === 'list-item') return createListFromPrefix(editor, elem);

	const props: Partial<Element> = { ...elem.data, type: elem.type, children: [] };
...
function modifiedDeleteBackward(editor: RichEditor, type: TextUnit): boolean {
	...
	if (!Element.isElement(block) || !Point.equals(selection.anchor, start)) return false;
	else if (type === 'word') return fullUnwrap(editor, block);
	else if ((block as Element).type === 'list-item') return unwrapList(editor, path);
	else if ((block as Element).type === 'table-cell') return removeColumn(editor, path);
	else if (blockContainers.includes((block as Element).type as EditorBlockElementType))
	...
	if (EditorItemParents[(block as Element).type as EditorItemElementType])
		Transforms.unwrapNodes(editor, {
			match: (n) =>
				!Editor.isEditor(n)
				&& Element.isElement(n)
				&& EditorItemParents[block.type as EditorItemElementType].includes(
					n.type as EditorBlockElementType,
				),
			split: true,
		});
...
function fullUnwrap(editor: RichEditor, block: Element) {
	Transforms.unwrapNodes(editor, {
		...
		mode: 'all',
		match: (node) =>
			Element.isElement(node) && node.type !== 'paragraph' && node.type !== 'code-line',
	});

	if (block.type === 'code-line') Transforms.setNodes(editor, { type: 'paragraph' });
...
function unwrapList(editor: RichEditor, path: Path): boolean {
	...
	const grandparentIsList =
		Element.isElement(grandparent?.[0])
		&& EditorListElementType.includes(grandparent[0].type as EditorListElementType);

	if (!grandparentIsList) Transforms.unwrapNodes(editor, { at: path, split: true });

	Transforms.unwrapNodes(editor, { at: parent, split: true });
...
function removeColumn(editor: RichEditor, path: Path): boolean {
	...
	if (table![0].children[0]!.children.length === 1) {
		editor.removeNodes({ at: table[1] });

		// Make sure a node at least exists
		if (!editor.children.length) editor.insertNode(paragraph());
...
export default function withCgMarkdown(editor: RichEditor) {
	...
	editor.insertText = (text) => {
		if (!modifiedInsertText(editor, text)) insertText(text);
	};

	editor.deleteBackward = (type) => {
		const modified = modifiedDeleteBackward(editor, type);

		if (!modified) deleteBackward(type);
	};
