...
export default class HTTPClient {
    ...
    public fetchPDS<T>(method: HTTPMethodXRPC, nsid: string, request: Omit<RequestConfig, "route" | "method">) {
        return this.fetch<T>({ route: nsid, method, request: { headers: { "atproto-proxy": "" } }, ...request });
    }
    public getRecord<T extends AtprotoValueBase>(config: { repo: string; rkey: string; collection: string; }) {
        return this.fetchPDS<AtprotoRecord<T>>("GET", "com.atproto.repo.getRecord", { queries: config });
    }
    public getRecordList<T extends AtprotoValueBase>(config: { repo: string; collection: string; }) {
        return this.fetchPDS<GetRecordListResponse<T>>("GET", "com.atproto.repo.listRecords", { queries: config });
    }
    public putRecord<T>(config: { repo: string; rkey: string; collection: string; record: T; }) {
    ...
    public fetchProfile(actor: string) {
