...
const nodeSerializers: Record<
	EditorElementType,
	(element: Element) => RootContentMap[keyof RootContentMap] | TableRow | TableCell
> = {
	paragraph(element) {
	...
	['block-quote'](element) {
		return {
			type: 'blockquote',
			children: element.children.map(mdastifyNode) as (BlockContent | DefinitionContent)[],
		};
	},
	['code-block'](element) {
	...
	table(element) {
		...
		return {
			type: 'table',
			align: table.align,
			children: table.children.map(mdastifyNode) as TableRow[],
		};
	},
	image(element) {
		const { url, title, children } = element as EditorImage;

		return { type: 'image', url, title, alt: children.map((node) => Node.string(node)).join('') };
	},
	['table-row'](element) {
		return {
			type: 'tableRow',
	...
	['ordered-list'](element) {
		...
		return {
			...
			ordered: true,
			spread: true,
			children: element.children.map(mdastifyNode) as ListItem[],
		};
	},
	['list-item'](element) {
		return {
	...
	heading(element) {
		...
		return {
			type: 'heading',
			depth: (heading.depth ?? 1) as 1 | 2 | 3 | 4 | 5 | 6,
			children: element.children.map(mdastifyNode) as PhrasingContent[],
		};
	},
	link(element) {
...
const nodeDeserializers: Record<
	keyof RootContentMap,
	(node: MdastNode, nesting: ContentWithUnderline[]) => Node | Node[]
> = {
	paragraph(element, nesting) {
		return {
			type: 'paragraph',
			children: (element as Paragraph).children.flatMap((x) =>
				slatefyElement(x, nesting),
			) as unknown as EditorText[],
		};
	},
	definition(element, nesting) {
		...
		return [
			{
				type: 'paragraph',
				children: [slatefyText(definition.title ?? definition.label ?? definition.identifier, nesting)],
			},
			{
				type: 'paragraph',
				children: (element as Paragraph).children.flatMap((x) =>
	...
	blockquote(element, nesting) {
		return {
			type: 'block-quote',
			children: (element as Blockquote).children.flatMap((x) =>
				slatefyElement(x, nesting),
	...
	code(element) {
		...
		return {
			...codeBlock,
			type: 'code-block',
			children: codeBlock.value
				.split('\n')
				.map((x) => ({ type: 'code-line', children: [{ text: x }] })),
		};
	},
	list(element, nesting) {
		...
		return {
			...
			children: list.children.flatMap((x) => slatefyElement(x, nesting)) as EditorListItem[],
		};
	},
	table(element, nesting) {
		...
		return {
			type: 'table',
			align: table.align as BlockAlignment[] | null | undefined,
			children: table.children.flatMap((x) => slatefyElement(x, nesting)) as EditorTableRow[],
		};
	},
	tableRow(element, nesting) {
		...
		return {
			type: 'table-row',
			children: tableRow.children.flatMap((x) => slatefyElement(x, nesting)) as EditorTableCell[],
		};
	},
	tableCell(element, nesting) {
		...
		return {
			type: 'table-cell',
			children: tableCell.children.flatMap((x) => slatefyElement(x, nesting)) as EditorText[],
		};
	},
	listItem(element, nesting) {
		...
		return {
			type: 'list-item',
			children: listItem.children.flatMap((x) => slatefyElement(x, nesting)) as (
				| EditorBlockElement
				| EditorText
			)[],
		};
	...
	heading(element, nesting) {
		...
		return {
			type: 'heading',
			depth: heading.depth,
			children: heading.children.flatMap((x) => slatefyElement(x, nesting)) as EditorText[],
		};
	},
	...
	image(element, nesting) {
		...
		return [
			slatefyText('!', nesting),
			{
				...
				url: image.url,
				title: image.title,
				children: [slatefyText(image.alt ?? '', nesting)],
			},
		];
	},
	...
	linkReference(element, nesting) {
		...
		return {
			...
			url: link.identifier,
			title: link.label,
			children: link.children.flatMap((x) => slatefyElement(x, nesting)) as EditorText[],
		} satisfies EditorLink;
	},
	...
	yaml(element, nesting) {
		...
		return [
			...
			{ type: 'paragraph', children: [slatefyText(yaml.value, nesting)] },
			{ type: 'divider', children: [{ text: ' ' }] },
		];
	},
	break() {
