...
const TentMessageWrapper = styled(Stack, { name: 'TentMessage', slot: 'root' })(({ theme }) => ({
	padding: '12px 24px',
	backgroundColor: 'transparent',
	transition: 'background 0.3s',
	position: 'relative',
	'&.waiting': { opacity: 0.45 },
	'&.error': { color: theme.vars.palette.danger[500] },
	'&.being-replied-to': { backgroundColor: theme.vars.palette.info[950] },
	'&.being-replied-to:hover': { backgroundColor: theme.vars.palette.info[900] },
	...
	'&:not(.unhoverable):hover': { backgroundColor: theme.vars.palette.background.level2 },
}));

const TentMessageReplies = styled(Box, { name: 'TentMessage', slot: 'replies' })(() => ({}));

const TentMessageContainer = styled(Stack, { name: 'TentMessage', slot: 'container' })(() => ({
	flexDirection: 'row',
	gap: '16px',
}));
...
type MessageTypeComponentProps = Pick<
	Props,
	'message' | 'colorRoles' | 'waiting' | 'error' | 'onUserClick'
>
	& React.PropsWithChildren;

const TentMessageComponentByType: Record<
	TentMessageType,
	(props: MessageTypeComponentProps) => ReactNode[] | ReactNode
> = { default: TentMessageDefault, system: TentMessageSystem };

export default function TentMessage({
	unhoverable,
	waiting,
	error,
	onUserClick,
	...
	const onEdit = (content: string) => {
		...
		return api.messages.update(message.tentId, message.id, { content }).then((resp) => {
			if (!resp.ok) return;
	...
	return (
		<TentMessageWrapper
			...
			className={`TentMessage-wrapper${unhoverable ? ' unhoverable' : ''}${isBeingRepliedTo ? ' being-replied-to' : ''}${waiting ? ' waiting' : ''}${error ? ' error' : ''}`}
		>
			{!hideToolbar && hover && (
				<MessageToolbar
	...
	return (
		<>
			<Box onClick={onAuthorClick}>
				<UserAvatar
					did={message.createdBy.user.did}
					avatar={message.createdBy.user.avatar}
					size='lg'
				/>
			</Box>
			<Stack flex={1}>
						...
						<Tooltip title={new Date(message.updatedAt).toLocaleString('en-US', defaultDateOptions)}>
							...
							>
								<FormattedMessage
									...
									values={{ icon: <IconPencil size={16} /> }}
								/>
							</Typography>
						</Tooltip>
					)}
	...
	return (
		<Group gap={1}>
			...
			<Typography
				...
				fontWeight={700}
			>
				<Skeleton loading>??? hours ago</Skeleton>
			</Typography>
		</Group>
	);
}
...
function TentMessageSkeletonContent() {
	return (
		<Box>
			<Typography>
				<Skeleton loading>{loremIpsum.lg}</Skeleton>
			</Typography>
		</Box>
	);
}
