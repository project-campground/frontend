...
const TooltipAnchor = styled('div', { name: 'TooltipAnchor', slot: 'root' })(() => ({
	position: 'absolute',
	width: 0,
	height: 2,
}));

export default function RichEditorFloater({
	useRelativeRef,
}: {
	useRelativeRef: RefObject<HTMLDivElement | null>;
}) {
	...
	useEffect(() => {
		if (!ref.current || !useRelativeRef.current) return;
		else if (
			!focused
			|| !editor.selection
