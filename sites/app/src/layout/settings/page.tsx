...
export default function SettingsPageWrapper({ children, header, subheader, startDecorator, endDecorator, ...props }: Props) {
    return (
        <SettingsPage>
            <Stack sx={{ px: 3, py: 2, }}>
                <Typography level="title-lg" startDecorator={startDecorator} endDecorator={endDecorator}>{header}</Typography>
                {subheader}
            </Stack>
