...
export function SnackbarContextProvider({ children }: React.PropsWithChildren) {
	...
	const notify = (props: SnackbarProps) =>
		setSnackbars([...snackbars, { ...props, id: Math.floor(Math.random() * 10000) }]);
	const notifyError: SnackbarNotify = (text) =>
		notify({ color: 'danger', startDecorator: <IconExclamationCircleFilled />, text });
	const notifyApiError: SnackbarNotifyApi<HttpResponseError> = (resp: HttpResponseError) =>
		notify({
			color: 'danger',
	...
	const notifyInfo: SnackbarNotify = (text) =>
		notify({ color: 'info', startDecorator: <IconInfoCircleFilled />, text });
	const notifyWarn: SnackbarNotify = (text) =>
		notify({ color: 'warning', startDecorator: <IconExclamationCircleFilled />, text });
	const removeSnackbar = (snackbar: SnackbarItem) =>
			...
			{snackbars.map((x) => {
				...
				return (
					<Snackbar
						...
						sx={{ border: 'solid 1px var(--neutral-border)' }}
					>
						{x.text}
					</Snackbar>
				);
