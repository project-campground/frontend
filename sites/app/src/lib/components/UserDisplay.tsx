...
const sizeToGap: Record<Size, number> = { sm: 1, md: 1.5, lg: 2 };
...
export function UserDisplayNoModal<T extends ProfileViewEmpty>({
	onClick,
	noUsernameDisplay,
	withStatus,
	noAvatar,
	...
	showHandle,
	member,
}: Props<T> & { onClick?: (ev: MouseEvent<HTMLDivElement>) => unknown }) {
			...
			{!noAvatar && (
				<>
					<UserAvatar
						...
						did={user.did}
						size={avatarSize ?? actualSize}
					/>
				</>
			)}
			{!noUsernameDisplay && (
				<UserDisplayUsername alignItems={align}>
					<TextBlock align={align}>
						<GradientTypography
							motion={motion ?? 'none'}
							colors={colors}
							level={`title-${actualSize}`}
							fontWeight={700}
						>
			...
			<Dropdown>
				<MenuButton
					...
					sx={{
						...
						':hover': { backgroundColor: props.noHoverBackground ? 'transparent' : undefined },
					}}
				>
...
export function UserDisplaySkeleton({
	showHandle,
	withStatus,
	noAvatar,
	size,
	...
	Props<ProfileViewEmpty>,
	'showHandle' | 'withStatus' | 'noAvatar' | 'size' | 'alignItems' | 'avatarSize'
>) {
	const actualSize = size ?? 'md';

	return (
		<Group
			gap={sizeToGap[actualSize]}
		...
		>
			...
			<Typography
				...
				fontWeight={700}
			>
				<Skeleton loading>Example user</Skeleton>
			</Typography>
			{showHandle && (
				<>
					<Typography
