...
type Item = { valid: boolean; value: any; id: number };

export interface FormFieldArrayProps<TValue, TProps extends FormFieldProps<TValue>>
	extends FormFieldProps<any[]>, FormFieldDecoratorProps {
	FieldComponent: { new (props: TProps, context: FormContext): AbstractFormField<TValue, TProps> };
	fieldProps: Omit<TProps, 'id'>;
	max?: number;
	min?: number;
}

type State = { items: Item[]; value: any[] };

export default class FormFieldArray<
	TValue,
	TProps extends FormFieldProps<TValue>,
> extends AbstractFormField<TValue[], FormFieldArrayProps<TValue, TProps>, State> {
	private _resetValueHandlers: Record<string | number, ResetValueHandler> = {};
	constructor(props: FormFieldArrayProps<TValue, TProps>, context: FormContext) {
		super(props, context, [], {
			items: FormFieldArray.createDefaultItems<TValue, TProps>(props.defaultValue, props.fieldProps),
		});
	}
	...
	private static createDefaultItems<TValue, TProps extends FormFieldProps<TValue>>(
		defaultValue: any[] | undefined | null,
		fieldProps: Omit<TProps, 'id'>,
	) {
		return (
			defaultValue?.map((value) =>
				FormFieldArray.createItem(value, FormFieldArray.isDefaultValid<TValue, TProps>(fieldProps)),
			) ?? []
		);
	}

	private static createItem(value: any, valid: boolean) {
		return { id: this.getItemId(), value, valid } satisfies Item;
	}

	private static isDefaultValid<TValue, TProps extends FormFieldProps<TValue>>(
		fieldProps: Omit<TProps, 'id'>,
	) {
		return (
			!fieldProps.required
			|| (typeof fieldProps.defaultValue !== 'undefined' && fieldProps.defaultValue !== null)
		);
	}

	public override get isValid(): boolean {
		return (
			this.state.items.slice(0, this.state.value.length).every((x) => x.valid)
	...
	private onSubFieldChange = (id: number, isValid: boolean, value: any) => {
		...
		this.setState({ value: this.state.items.map((x) => x.value) }, () => this.onValueChange());
	};
		...
		this.setState({
			...
			items: [
				...
				FormFieldArray.createItem(
					...
					FormFieldArray.isDefaultValid<TValue, TProps>(this.props.fieldProps),
				),
			],
		});
	}
		...
		this.setState(
			{
				...
				items: moveIndexes(this.state.items, draggedIndex, droppedAtIndex),
			},
			() => this.onValueChange(),
		);
	}
			...
			>
				<FormContext.Provider
					value={{
						...
						validFields: Object.fromEntries(items.map(({ id, valid }) => [id, valid])) as Record<
							number,
							boolean
						>,
				...
				>
					<Stack gap={1}>
						<DragDropProvider
							onDropped={(draggedId, droppedId) => this.onValueMove(Number(draggedId), Number(droppedId))}
						>
							{items.map((x, i) => (
								<FormFieldArrayItem
									key={x.id}
				...
				<Button
					...
					disabled={!!this.props.max && value.length >= this.props.max}
				>
					Add
				</Button>
			</Stack>
...
const FormFieldArrayItemCard = styled(Card)(({ theme }) => ({
	...
	'&.over::after': { backgroundColor: theme.vars.palette.primary[500] },
}));
...
function FormFieldArrayItem<TValue, TProps extends FormFieldProps<TValue>>({
	...
	defaultValue,
}: Pick<FormFieldArrayProps<TValue, TProps>, 'FieldComponent' | 'fieldProps'> & {
	onRemove: () => unknown;
	id: number;
	defaultValue: any;
}) {
	const { attributes: draggableAttributes } = useDraggable({ id: id.toString() });
	const { attributes: droppableAttributes, isOver } = useDroppable({ id: id.toString() });

	const props = { id, defaultValue, ...fieldProps } as Readonly<TProps>;
