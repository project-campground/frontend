...
const ThreadItemHook = styled(`div`, { name: 'ThreadLineItem', slot: 'hook' })(({ theme }) => ({
	position: 'absolute',
...
const ThreadItemLine = styled(`div`, { name: 'ThreadLineItem', slot: 'line' })(({ theme }) => ({
	position: 'absolute',
	border: `solid 3px ${theme.vars.palette.neutral[700]}`,
	borderTop: 0,
	borderRight: 0,
	...
	width: 3,
	left: 20,
	'&:not(.reverse)': { bottom: 0 },
	'&.reverse': { top: 0 },
}));

const ThreadLineItemWrapper = styled(Group, { name: 'ThreadLineItem', slot: 'root' })(() => ({
	position: 'relative',
	width: '100%',
	'&:not(.reverse):last-of-type .line': { opacity: 0 },
	'&.reverse:first-of-type .line': { opacity: 0 },
}));

export function ThreadLineItem({
	children,
	reverse,
	hookSx,
}: React.PropsWithChildren & { reverse?: boolean; hookSx?: SxProps }) {
	return (
		<ThreadLineItemWrapper className={`ThreadLineItem-wrapper ${reverse ? 'reverse' : ''}`}>
			...
			<Box sx={{ ml: 5, flex: 1 }}>{children}</Box>
