...
export default class PermissionsManager {
	...
	private onNewTentList(value: GetTentsOutput | null) {
		for (const perm in this._tentToPermissions) delete this._tentToPermissions[perm];

		if (this.campsite.me.user.did === this.campsite.owner) return;

		this.permissions = aggregateAllPermissions(
			this.campsite.me,
			this.campsite.roles,
			value?.permissions ?? [],
		);
	...
	public getTentPermissions(categoryId: string | null | undefined, tentId: string) {
		if (this.campsite.me.user.did === this.campsite.owner) return maxPermissions;
		else if (this._tentToPermissions[tentId]) return this._tentToPermissions[tentId];

		const categoryPerms =
			(categoryId ? this.permissions.categories[categoryId] : undefined) ?? this.permissions.bonfire;
		const tentPermsState = this.permissions.tents[tentId];

		if (!tentPermsState) return categoryPerms;

		return ((this._tentToPermissions[tentId] as PermissionsDictionary) = {
			general:
				(categoryPerms.general & invertGeneralPermission(tentPermsState.denied.general))
				| categoryPerms.general,
			content:
				(categoryPerms.content & invertContentPermission(tentPermsState.denied.content))
				| categoryPerms.content,
		});
	}
}
