...
export default function TentCreationModal({
	campsiteId,
	bonfireId,
	categoryId,
	lowestPriorityTent,
	...
	const onTentCreate = (body: Record<string, any>): unknown =>
		api.tents
			.create(campsiteId, bonfireId, {
				...body,
				categoryId: categoryId ?? undefined,
				position: lowestPriorityTent + 1,
			} as { categoryId?: number; name: string; type: number; description: string; position: number })
			.then((r) => {
	...
	const onCategoryCreate = (name: string, description: string): unknown =>
		api.categories
			.create(campsiteId, bonfireId, { name, description, position: lowestPriorityCategory + 1 } as {
				name: string;
				description: string;
				position: number;
			})
	...
	return (
		<ModalDialog>
			...
			<Form
				inlineContent
				onSubmit={(_, { what, ...values }) =>
					what === 'category' ? onCategoryCreate(values.name, values.description) : onTentCreate(values)
				}
			>
				<FormSection>
					<FormSection>
					...
					{({ values: { description, what, name } }) => (
						...
						>
							...
							>
									...
									header={
										<Stack flex={1}>
											<Typography level='title-md'>
												{what === 'category' ? name : 'Example category'}
											</Typography>
											{description && (
												<Typography level='body-sm'>
													{what === 'category' ? description : 'Example description'}
												</Typography>
											)}
										</Stack>
									}
								>
									<TentList>
										<PseudoTentItem tent={{ name: 'Example tent #1', type: 'text', viewType: 0 }} />
										<PseudoTentItem
											isActive={what === 'tent'}
											tent={{ name: what === 'tent' ? name : 'Example tent #2', type: 'text', viewType: 0 }}
										/>
										<PseudoTentItem tent={{ name: 'Example tent #3', type: 'text', viewType: 0 }} />
									</TentList>
								</ContentCategory>
							</Card>
						</Sheet>
