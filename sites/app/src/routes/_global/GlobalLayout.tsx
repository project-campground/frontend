...
import type { CampsiteViewBasic, CampsiteViewWithDomain } from 'types/campground/campsites';
...
type Props = { page: string | undefined | null; children: ReactNode[] | ReactNode };
...
const GlobalLayoutRoot = styled(Box, { name: 'GlobalLayout', slot: 'root' })(({ theme }) => ({
	display: 'grid',
...
export default class GlobalLayout extends React.Component<Props, State> {
	...
	state: State = { loadedSessionInfo: false, loadedCampsites: false, userSettingsOpen: false };
	...
	private async onInitPreferences(): Promise<unknown> {
		...
		const backendDomains = [
			...new Set(
				(campsitesPref?.campsites.map((x) => x.split('@')[0]) ?? []).concat(defaultBackendDomain),
			),
		];

		return Promise.allSettled(
			backendDomains.map((domain) => this.context.atproto.getBackendJoinedCampsites(domain)),
	...
	async componentDidMount(): Promise<void> {
		...
		if (!this.context.auth.authenticated) return this.setState({ loadedSessionInfo: true });
		...
		return Promise.all([
			...
			// Deleted account
			if (sessionResp.status === 400) return this.onGetSessionError(sessionResp.errorDescription!);
	...
	onGetSessionError(message: string) {
		this.setState({ errorLoading: message });
	}
	...
	onWsMessage<T extends keyof TypeToPayload>(type: T, payload: TypeToPayload[T]) {
		switch (type) {
			...
			case 'CampsiteUpdated':
				...
				this._campsites.push({ ...payload, _domain: defaultBackendDomain } as CampsiteViewWithDomain);
				break;
	...
	render() {
		...
		const { loadedSessionInfo: loaded, userSettingsOpen, errorLoading } = this.state;
			...
			<GlobalLayoutRoot>
					...
					value={
						loaded ?
							this.context.auth.authenticated ?
								{
									authenticated: true,
									profile: this._me ?? ({} satisfies Partial<CampgroundProfileRecord>),
				...
				>
					...
					<Modal
						...
						onClose={() => this.setState({ userSettingsOpen: false })}
					>
						<UserSettingsModal />
					</Modal>
				</AccountContext.Provider>
