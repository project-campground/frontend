...
type State = { value: string; errorMessage?: string | null | undefined; menuOpen: boolean };
...
export default class FormFieldText extends AbstractFormField<string, FormFieldTextProps, State> {
	...
	private onInputChange(ev: ChangeEvent<HTMLInputElement>) {
		...
		if (this.props.max && value.length > this.props.max) return;
	...
	private onNewValue(value: string, onNewValue?: () => void) {
		...
		this.setState({ value, errorMessage: undefined }, () =>
			onNewValue ? onNewValue() : this.onValueChange(),
		);
	}

	public override render(): ReactNode {
		const { startDecorator, endDecorator, placeholder, inputType, disabled, knownValues } =
			this.props;
		const {
			hasValidValue,
			state: { value, errorMessage, menuOpen },
		} = this;
					...
					{knownValues && (
						...
						>
							{knownValues.map((x) => (
								<MenuItem
									key={x.value}
									onClick={() => (console.log({ x }), this.onNewValue(x.value))}
								>
