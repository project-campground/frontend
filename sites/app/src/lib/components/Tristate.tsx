...
const TristateRoot = styled(Stack, { name: 'Tristate', slot: 'root' })<{
...
const TristateLabel = styled(Box, { name: 'Tristate', slot: 'label' })<{
	ownerState: TristateOwnerState;
}>(() => ({ flex: 1 }));

const TristateBox = styled(ButtonGroup, { name: 'Tristate', slot: 'box' })<{
	ownerState: TristateOwnerState;
}>(({ theme }) => ({
	position: 'relative',
	overflow: 'hidden',
...
const TristateBoxBackground = styled(Box, { name: 'Tristate', slot: 'box-background' })<{
	ownerState: TristateOwnerState;
}>(({ theme }) => ({
	position: 'absolute',
	height: '100%',
	...
	'&.on': { left: '66.66%', backgroundColor: theme.vars.palette.success[900] },
	'&.pass': { left: '33.33%', backgroundColor: theme.vars.palette.warning[900] },
	'&.off': { left: '0%', backgroundColor: theme.vars.palette.danger[900] },
}));
...
const TristateButton = styled(IconButton)(({ theme, color }) => ({
	...
	'&.checked': { color: theme.vars.palette[color as 'neutral'][500] },
	'&.checked:hover': {
		backgroundColor: 'rgba(255, 255, 255, 0.05)',
	...
	return (
		...
		>
			<TristateBox
				...
				size='md'
			>
				<TristateBoxBackground
					ownerState={ownerState}
					className={triValue}
				/>
				<TristateButton
					className={triValue === 'off' ? 'checked' : ''}
					onClick={onValueChange.bind(null, 'off')}
					variant='plain'
					color='danger'
				>
					<IconX />
				...
				<TristateButton
					className={triValue === 'pass' ? 'checked' : ''}
					onClick={onValueChange.bind(null, 'pass')}
					variant='plain'
					color='warning'
				>
					<IconSlash />
				...
				<TristateButton
					className={triValue === 'on' ? 'checked' : ''}
					onClick={onValueChange.bind(null, 'on')}
					variant='plain'
					color='success'
				>
					<IconCheck />
			...
			{label && <TristateLabel ownerState={ownerState}>{label}</TristateLabel>}
		</TristateRoot>
	);
});
