...
const ColorPick = styled(Box, { name: 'ColorPick', slot: 'root' })(({ theme }) => ({
	width: 24,
	height: 24,
...
const preselectedColors = [0xfe1c56, 0xff5a26, 0xfea01c, 0x0cef43, 0x26f2ff, 0x6026ff, 0xca1cfe];
const dark = 0x0e0b16;
		...
		return (
			<Stack>
				<Group gap={1}>
					<Dropdown>
						<MenuButton
							sx={(theme) => ({ boxShadow: theme.vars.shadow.sm, fontFamily: theme.fontFamily.code })}
							color='neutral'
							variant='outlined'
							startDecorator={<ColorPick sx={{ backgroundColor: hexColor }} />}
						>
							{hexColor}
						</MenuButton>
						<ColorPickerMenu
					...
					<Stack gap={1}>
						<Group gap={1}>
							{preselectedColors.map((color) => (
								<ColorPick
									key={color}
									onClick={() => this.onInputChange(color)}
									sx={{ backgroundColor: `#${color.toString(16).padStart(6, '0')}` }}
								/>
							))}
						</Group>
						<Group gap={1}>
