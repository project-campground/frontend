...
const settingsPages: Record<
	...
	| ((props: SettingsComponentProps<CampsiteSettingsProps>) => ReactNode | ReactNode[])
> = {
...
export type CampsiteSettingsProps = { campsite: CampsiteViewDetailed };
...
export default function CampsiteSettingsModal(props: CampsiteSettingsProps) {
	...
	const callbacks: Record<Page, (fieldValues: Record<string, any>) => unknown> = {
		...
		roles: ({ id, ...fieldValues }) =>
			api.roles.update(props.campsite.id, id as string, fieldValues).then((resp) => {
				if (!resp.ok) return snackbars.notifyApiError(resp);

				const modifiedRole = props.campsite.roles.find((x) => x.id === resp.content.id);
				if (modifiedRole) return Object.assign(modifiedRole, resp.content);
			}),
		bans: () => null,
		invites: () => null,
		delete: () => null,
