...
// Code wrapping
export const CodeContainer = styled('div', { name: 'CodeContainer', slot: 'container' })(
	({ theme }) => ({
		position: 'relative',
		...
		'> .code-icon-button': { opacity: 0, transition: 'opacity 0.5s' },
		':hover > .code-icon-button': { opacity: 1 },
		'*.token.operator': { color: theme.vars.palette.text['code-keyword'] },
		'*.token.string': {
		...
		'*.token.comment': { color: theme.vars.palette.neutral[400] },
...
export const CodePre = styled('pre', { name: 'CodePre', slot: 'pre' })(() => ({ margin: 0 }));
export const CodeGrid = styled('code', { name: 'CodeGrid', slot: 'grid' })(() => ({
	position: 'relative',
	display: 'grid',
...
export const CodeHeader = styled('header', { name: 'CodeHeader', slot: 'header' })(({ theme }) => ({
	display: 'flex',
	flexDirection: 'column',
	userSelect: 'none',
	gap: 4,
...
export const CodeLanguage = styled(Chip, { name: 'CodeLanguage', slot: 'language' })(
	({ theme }) => ({
...
// Code lines
export const CodeLineNumber = styled('div', { name: 'CodeLineNumber' })(({ theme }) => ({
...
export const CodeLine = styled('div', { name: 'CodeLine' })(({ theme }) => ({
	paddingLeft: 16,
	paddingRight: 16,
	'&.highlighted': { backgroundColor: theme.vars.palette.info[900] },
}));

const splitTokensWithScope = (
	value: string | HighlightToken,
	scope: string,
): Array<0 | { scope: string; text: string }> =>
	typeof value === 'string' ?
	...
	:	value.children.flatMap((x) => splitTokensWithScope(x, value.scope));

const splitTokens = (
	value: string | HighlightToken,
): Array<0 | string | { scope: string; text: string }> =>
	typeof value === 'string' ?
		insertLineBetween(value.split('\n')).filter((x) => x || x === 0)
...
function insertLineBetween<T>(arr: T[] | string[]): Array<string | 0 | T> {
	...
	const withElemsAfter = arr.slice(1).flatMap((x) => [0, x]) as (string | T | 0)[];
...
export function linefyTokens<T>(arr: (0 | T)[]): T[][] {
	...
	return arr.reduce(
		(a: T[][], b: 0 | T) =>
			typeof b === 'number' ? [...a, []] : [...a.slice(0, a.length - 1), [...a[a.length - 1], b]],
		[[]],
	);
}
...
export default class CodeBlock extends React.Component<Props> {
	...
	static tokenizeContent(language: string | null | undefined, content: string) {
		...
		if (!language || CodeBlock.nonHighlightedLanguages.includes(language))
			return { tokens: insertLineBetween(content.split('\n')) };
		...
		return { language: { language, name }, tokens: nodes.flatMap(splitTokens) };
	}
	...
	static splitByCodeLines(
		tokenizedContent:
			| { language: undefined; tokens: (string | 0)[] }
			| {
					language: { language: string; name: string | undefined };
					tokens: (string | 0 | { scope: string; text: string })[];
			  },
	) {
		return {
			language: tokenizedContent.language,
			tokens: linefyTokens(tokenizedContent.tokens.map(componentifyToken)),
		};
	}
	...
	render() {
		...
		const noLanguage =
			CodeBlock.nonHighlightedLanguages.includes(languageDisplayName) && !overrideLanguageDisplayName;
		...
		return (
			<CodeContainer>
				<IconButton
					className={noLanguage ? `code-icon-button no-language` : `code-icon-button with-language`}
					sx={{ position: 'absolute', right: iconButtonMargin, top: iconButtonMargin }}
					size='sm'
				...
				{(!noLanguage || description) && (
					<CodeHeader>
						{!noLanguage && (
							<Stack flex={1}>
								<CodeLanguage>
									{overrideLanguageDisplayName?.substring(0, 64) ?? languageDisplayName}
								</CodeLanguage>
							</Stack>
						)}
						{description && (
						...
						{linefied.tokens.map((x, i) => {
							...
							return (
								<React.Fragment key={i}>
									<CodeLineNumber className={commonClassNames}>{i + startingLine}</CodeLineNumber>
