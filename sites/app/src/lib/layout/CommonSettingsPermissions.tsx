...
function createNewPermission({
	...
	return {
		...
		roleId,
		userId,
		permissions: { allowed: { general: 0, content: 0 }, denied: { general: 0, content: 0 } },
	};
}
...
export default function CommonSettingsPermissions({
	...
	useMemo(() => {
		api.permissions
			.get({ non_self: true, tent_id: tentId, bonfire_id: bonfireId, category_id: categoryId } as
				| { bonfire_id: string }
				| { tent_id: string }
				| { category_id: string })
			.then((resp) => {
				...
				const permissionsReceived = resp.content.permissions as CampsitePermissionViewBasic[];
				...
				const defaultRolePermission = permissionsList.find((x) => x.roleId === defaultRole.id) as
					| CampsitePermissionViewSettings
					| undefined;
				...
				setPermissions(
					defaultRolePermission ? permissionsList : (
						(permissionsList as CampsitePermissionViewSettings[]).concat(nonNullDefault)
					),
				);
	...
	const onCreateRolePermission = (role: RoleView) => {
		...
		const newPermissionList =
			openPermission.new && openPermission.roleId !== defaultRole.id ?
				permissions.filter(
					(x) => x.userId !== openPermission.userId || x.roleId !== openPermission.roleId,
				)
			:	permissions;
	...
	const changeOpenPermission = (permission: CampsitePermissionViewSettings) => {
		...
		if (oldOpenPermission.new && oldOpenPermission.roleId !== defaultRole.id)
			return setPermissions(
				permissions.filter(
					(x) => x.userId !== oldOpenPermission.userId || x.roleId !== oldOpenPermission.roleId,
				),
			);
	};
	...
	return (
						...
						{roles
							...
							.map((x) => (
								...
								>
									...
									<ListItemContent>
										<GradientTypography colors={colorToDecimal(x.colors)}>{x.name}</GradientTypography>
									</ListItemContent>
								</MenuItem>
							))}
					</Menu>
		...
		>
			<Stack
				sx={{ width: { xs: 128, lg: 256 }, height: '100%' }}
				gap={2}
			>
				<Stack sx={{ height: '100%' }}>
					{permissions
...
function getTristateValue(
	...
	return (
		(combinedValues.permissions.allowed[type] & value) === value ? 'on'
		: (combinedValues.permissions.denied[type] & value) === value ? 'off'
		: 'pass'
	);
}

function PermissionsPage({ permission, role, onChanged, isBonfire }: PermissionsPageProps) {
	const {
		defaultValues,
		combinedValues,
	}: {
		...
		() => ({
			defaultValues: { permissions: permission.permissions },
			combinedValues: {
								...
								>
									...
									<FormControl>
										<FormFieldTristate
											...
											label={<FormattedMessageGlobal id='app.permissions.managePermissions' />}
											description={<FormattedMessageGlobal id='app.permissions.managePermissions.desc' />}
											defaultValue={getTristateValue(
												combinedValues,
												'general',
