...
const sizeToPx: Record<Size, number> = { sm: 24, md: 32, lg: 48, xl: 56, xxl: 80, xxxl: 128 };
...
const StyledAvatar = styled(Avatar)<{ size?: Size }>(({ theme, size }) => ({
	...
	borderRadius:
		theme.vars.radius[
			(size as Size) === 'xxl' || (size as Size) === 'xxxl' ? 'xl' : ((size as 'md') ?? 'md')
		],
	// zIndex: 7,
}));
...
export function UserAvatarSkeleton({
	...
	badgeSx,
	sx,
}: Pick<Props, 'sx' | 'badgeSx' | 'size' | 'withStatus'>) {
	const sizePx = sizeToPx[size ?? 'md'];
	const badgeSize = sizePx * 0.25;
