import { ListItem, ListItemButton, ListItemContent, ListItemDecorator, MenuItem, Modal } from "@mui/joy";
...
export default function MemberItem({ campsiteId, member, roles, onClick }: Props) {
    const { permissions, campsite: { owner, me } } = useCampsiteContext();
        ...
        MenuComponent: () => (
            <>
                {(
                    ...
                    me.user.did === member.user.did && !!(mePermissions & GeneralPermissionConsts.MANAGE_SELF_IDENTITY)
                ...
                {meAboveUser && !!(mePermissions & GeneralPermissionConsts.KICK_MEMBERS) && <MenuItem variant="plain" color="danger" onClick={removeMember}>
                    <ListItemDecorator>
                        <IconUserMinus />
                    </ListItemDecorator>
                    <ListItemContent>
                ...
                {meAboveUser && !!(mePermissions & GeneralPermissionConsts.KICK_MEMBERS) && <MenuItem variant="plain" color="danger" onClick={() => setOpenModal("ban")}>
                    <ListItemDecorator>
                        <IconHammer />
                    </ListItemDecorator>
                    <ListItemContent>
    ...
    return (
        <ListItem sx={{ userSelect: "none" }}>
            ...
            <Modal open={openModal === "nickname"}>
                <ChangeNicknameModal campsiteId={campsiteId} member={member} onClose={closeModal} />
            </Modal>
            <Modal open={openModal === "ban"}>
                <BanMemberModal campsiteId={campsiteId} member={member} onClose={closeModal} />
            </Modal>
        </ListItem>
    );
}