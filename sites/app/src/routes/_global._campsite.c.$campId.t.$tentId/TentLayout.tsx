...
type Props = { campsiteId: string; tentId: string };
...
export default function TentLayout({ campsiteId, tentId }: Props) {
	...
	useMemo(async () => {
		if (PseudoTentType.includes(tentId as PseudoTentType))
			return setTentInfo({
				...
				name: intl.formatMessage({
					...globalIntlDeclarations[`app.tents.${tentId}` as 'app.tents.bulletin'],
				}),
				description: '',
				viewType: 0,
		...
		return api.tents.get(tentId).then((resp) => {
	...
	if (!tentInfo) return <TentLayoutSkeleton />;
	...
	session.ws.subscribe((message) =>
		(
			message.op === 1
			&& message.t === 'TentDeleted'
			&& (message.payload as TypeToPayload['TentDeleted']).id === tentInfo.id
		) ?
	...
	return (
		<>
			...
			<SidebarWrapper className='open'>
				<RightSidebarBox>
					<RightSidebarList sx={{ height: '100%' }}></RightSidebarList>
				</RightSidebarBox>
			</SidebarWrapper>
		</>
	);
