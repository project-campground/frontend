...
interface RightClickInternal<T> {
	onCreate: (props: {
		ItemComponent: (props: T) => ReactNode | ReactNode[];
		props: T;
		top: number;
		left: number;
	}) => unknown;
...
export function useRightClick<TElem, TProps>({
	MenuComponent,
	menuProps,
}: {
	menuProps: TProps;
	MenuComponent: (props: TProps) => ReactNode | ReactNode[];
}): RightClick<TElem> {
...
export function RightClickProvider({ children }: React.PropsWithChildren) {
	const [menu, setMenu] = useState<{
		ItemComponent: (props: any) => ReactNode | ReactNode[];
		props: any;
		top: number;
		left: number;
	} | null>(null);
	...
	return (
		<RightClickContext.Provider value={{ onCreate: setMenu }}>
			{children}
			{menu && (
				<ClickAwayListener onClickAway={removeMenu}>
					<Menu
						variant='soft'
						onClose={removeMenu}
						open={true}
