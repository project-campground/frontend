...
type TentItemView = Omit<TentView, 'name'> & { name: ReactNode[] | ReactNode };
...
export const TentItemButton = styled(ListItemButton, { name: 'TentItem', slot: 'root' })(
	({ theme }) => ({
		...
		'&.TentItem-over::after': { backgroundColor: theme.vars.palette.primary[500] },
		'&.TentItem-active': {
			color: theme.vars.palette.text.tertiary,
		...
		'&.TentItem-active:hover': { backgroundColor: theme.vars.palette.neutral[800] },
	}),
);
...
export default function TentItem({ isActive, tent, onSettingsOpen: onTentSettingsOpen }: Props) {
	...
	const { attributes: draggableAttributes } = useDraggable({
		...
		group: 'tent',
		disabled:
			(tentPermissions.general & GeneralPermissionConsts.MANAGE_TENTS)
		...
		MenuComponent: ({ tent }) => (
			<>
				...
				{!!(tentPermissions.general & GeneralPermissionConsts.MANAGE_TENTS)
					...
					&& onTentSettingsOpen && (
						<MenuItem onClick={() => onTentSettingsOpen({ tent })}>
							<ListItemDecorator>
								<IconSettingsFilled />
							</ListItemDecorator>
							<ListItemContent>
				...
				{!!(tentPermissions.general & GeneralPermissionConsts.MANAGE_TENTS)
					...
					&& onTentSettingsOpen && (
						<MenuItem
							color='danger'
							variant='plain'
							onClick={() => onTentSettingsOpen({ tent, page: 'delete' })}
						>
							<ListItemDecorator>
								<IconTrashFilled />
							</ListItemDecorator>
	...
	return (
		<ListItem {...listeners}>
			<TentItemButton
				{...draggableAttributes}
				{...droppableAttributes}
				className={[isActive && 'TentItem-active', isOver && 'TentItem-over']
					.filter((x) => x)
					.join(', ')}
				onClick={navigateToTent}
			>
				<ListItemDecorator>
					<TentIcon
						type={tent.type}
...
export function PseudoTentItem({
	isActive,
	tent,
}: Pick<Props, 'isActive'> & { tent: Pick<TentView, 'type' | 'viewType' | 'name'> }) {
	return (
		<ListItem>
