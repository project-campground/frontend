...
export interface FormFieldTristateFlagsProps
	extends PropsWithChildren, FormFieldProps<FormFieldTristateFlagsValue> {
	gap?: number;
	inline?: boolean;
}

type State = { value: FormFieldTristateFlagsValue; valid: Record<number, boolean> };
...
const oppositeTristateToFlag: Record<
	keyof PermissionsStateDictionary,
	keyof PermissionsStateDictionary
		...
		this.setState(
			{
				value:
					flagsName ?
						({
							[oppositeFlagsName!]:
								this.state.value[oppositeFlagsName!] & (this.state.value[oppositeFlagsName!] ^ id),
