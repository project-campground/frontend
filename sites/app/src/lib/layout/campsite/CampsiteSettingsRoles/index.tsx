...
export default function CampsiteSettingsRoles({
	...
	const roles = useMemo<SettingsRole[]>(() => campsite.roles, [campsite, campsite.roles]);
	...
	const moveRole = (roleMoved: string, movedTo: string) => {
		...
		if (
			!movedToIndex
			|| Math.abs(roles[movedToIndex - 1].position - roles[movedToIndex].position) > 1
		)
			return api.roles
				.moveMany(campsite.id, { rolesByPosition: { [roleMoved]: roles[movedToIndex]!.position - 1 } })
				.then(onRolesMoved);
		...
		const newPriorities = Object.assign(Object.fromEntries(rolesToAdditionallyMove), {
			[roleMoved]: newPriority,
		});

		return api.roles.moveMany(campsite.id, { rolesByPosition: newPriorities }).then(onRolesMoved);
	};
...
export type RolePageProps = {
	...
	role: SettingsRole;
	onChanged: (valid: boolean, changed: boolean, values: FormValues) => unknown;
};
...
function RolePage({ ref, onRoleDelete, role, onChanged }: RolePageProps) {
	const {
		defaultValues,
		combinedValues,
	}: { defaultValues: Omit<FormValues, 'id'>; combinedValues: FormValues } = useMemo(
		() => ({
			defaultValues: {
				name: role.name,
				colors: role.colors,
	...
	const onValuesChanged = (valid: boolean, values: Record<string, any>) => {
		...
		const permissionsChanged = defaultPermissionList.some(
			(x) => combinedValues.permissions[x] !== defaultValues.permissions[x],
		);
		const colorsChanged =
			defaultValues.colors.length !== combinedValues.colors.length
			|| defaultValues.colors.some((defaultColor, i) => combinedValues.colors[i] !== defaultColor);
