...
import BonfireListMenu from '../../lib/layout/sidebar/BonfireListMenu';
import { TentItemSkeleton } from '../../lib/components/tents/TentItem';
import { TentCategorySkeleton } from '../../lib/components/tents/TentCategory';
...
import InviteCreationModal from '../../lib/layout/InviteCreationModal';
...
export const TentSidebarBox = styled(Stack, { name: 'CampsiteSidebar', slot: 'root' })(
	({ theme }) => ({
...
export const TentSidebarBannerWrapper = styled(Box, { name: 'BonfireBanner', slot: 'banner' })(
	({ theme }) => ({
		borderRadius: theme.vars.radius.xl,
		overflow: 'hidden',
		position: 'relative',
...
const anyManageCampsitePermission =
...
export default class TentSidebar extends React.Component<Props, State> {
	...
	public get bonfireSelected(): BonfireViewBasic {
		return this.props.bonfireSelected ?
				(this.props.campsite.bonfires.find((x) => x.id === this.props.bonfireSelected)
					?? this.defaultBonfire)
			:	this.defaultBonfire;
	}
	...
	componentDidMount(): void {
		...
		this._wsSubscription = session.ws.subscribe(
			(msg) => msg.op === 1 && this.onWsEvent(msg.t as keyof TypeToPayload, msg.payload),
		);
		// FIXME
		this.setState({});
	}
	...
	componentWillUnmount(): void {
		...
		const { session } = this.context;
		return session.ws.unsubscribe(this._wsSubscription);
	}
	async componentDidUpdate(
		...
		else if (!this.state.loading || this._lock) return;

		const { bonfireSelected } = this.state;

		const tents = this.bonfiresToTents[bonfireSelected.id];
		if (tents) return this.setState({ loading: false });

		this._lock = true;
		return this.context.api.tents.getMany(this.props.campsite.id, bonfireSelected.id).then((x) => {
			if (!x.ok) return this.setState({ error: x });
	...
	leaveCampsite() {
		...
		return api.members
			...
			.then(
				(resp) =>
					resp && session.preferences.removeCampsiteFromListGlobally(api.domain, this.props.campsite.id),
			);
	}
	render(): React.ReactNode {
		...
		return (
			<TentSidebarBox>
					...
					>
						<TentSidebarBonfireDisplayBox onClick={toggleGroupMenu}>
							<TentSidebarTopBar>
								<Avatar
									sx={{ borderRadius: 'sm' }}
									src={bonfireSelected.avatarUri ?? undefined}
									size='sm'
									variant='solid'
									color='primary'
								>
								...
								<Stack sx={{ maxHeight: 45, width: 194 }}>
									...
									>
										{bonfireSelected.name}
									</Typography>
									{bonfireSelected.description && (
										<Tooltip
											variant='soft'
											title={bonfireSelected.description}
										>
											<Typography
												level='body-sm'
						...
						<Group>
							<Dropdown>
								...
								<Menu variant='soft'>
									{!!(permissions.role.general & GeneralPermissionConsts.CREATE_INVITES) && (
										<MenuItem
											variant='soft'
											onClick={this.setMenu.bind(this, 'invite-creation')}
										>
											<ListItemDecorator>
												<IconTicket />
											</ListItemDecorator>
									...
									{!!(permissions.role.general & anyManageCampsitePermission) && (
										<MenuItem
											variant='soft'
											onClick={this.setMenu.bind(this, 'campsite-settings')}
										>
											<ListItemDecorator>
												<IconCampfire />
											</ListItemDecorator>
									...
									{!!(permissions.bonfire.general & GeneralPermissionConsts.MANAGE_BONFIRES) && (
										<MenuItem
											variant='soft'
											onClick={this.setMenu.bind(this, 'bonfire-settings')}
										>
											<ListItemDecorator>
												<IconSettingsFilled />
											</ListItemDecorator>
									...
									{campsite.owner !== campsite.me.user.did && (
										<MenuItem
											variant='plain'
											color='danger'
											onClick={this.leaveCampsite.bind(this)}
										>
											<ListItemDecorator>
												<IconDoorExit />
											</ListItemDecorator>
				...
				/>
				<Modal
					open={menuOpen === 'invite-creation'}
					onClose={this.setMenu.bind(this, null)}
				>
					<InviteCreationModal campsiteId={campsite.id} />
				</Modal>
				<Modal
					open={menuOpen === 'campsite-settings'}
					onClose={this.setMenu.bind(this, null)}
				>
					<CampsiteSettingsModal campsite={campsite} />
				</Modal>
				<Modal
					open={menuOpen === 'bonfire-settings'}
					onClose={this.setMenu.bind(this, null)}
				>
