...
export function mdastMentions(tree: Root, _file: any, ...args: any[]) {
	...
	findAndReplace(tree, [
		...
		//     didMention
		// ],
		[/[<][@](did[:]plc[:][A-Za-z0-9]+)[>]/g, actorMention],
		[
...
type AnyMentionMdastElement =
	| ActorMentionMdastElement
	| TentMentionMdastElement
	| RoleMentionMdastElement;

function actorMention(_: any, did: string) {
	return { type: 'actorMention', id: did } satisfies ActorMentionMdastElement as unknown as Link;
}
function tentMention(_: any, id: string) {
	return { type: 'tentMention', id: id } satisfies TentMentionMdastElement as unknown as Link;
}
function roleMention(_: any, id: string) {
	return { type: 'roleMention', id: id } satisfies RoleMentionMdastElement as unknown as Link;
}
